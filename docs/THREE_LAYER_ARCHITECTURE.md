...
**位置**：`apps/official-sim-server/app/api/routes/mock_compat.py`
...
- `multiplatform_mock_openapi_zh/providers/` - 消费 platform-sim

**建议**：
- `platform-sim/providers/` 仅用于内部测试
- Omni-CSX 使用 `multiplatform_mock_openapi_zh/providers/`
...
cd multiplatform_mock_openapi_zh/apps/domain-service
