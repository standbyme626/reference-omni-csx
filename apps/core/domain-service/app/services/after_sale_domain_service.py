...
class AfterSaleDomainService:
    ...
    def get_after_sale_by_order(
        ...
        platform_enum = Platform(platform)
        try:
            refund_data = self.gateway.get_refund_by_order(
                platform_enum,
                order_id,
                official_run_id=official_run_id,
            )
    ...
    def batch_get_after_sales(self, requests: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        for req in requests:
            try:
                after_sale = self.get_after_sale(
                    req["platform"],
                    req["after_sale_id"],
    ...
    def _normalize_after_sale(
        ...
        external_order_id = raw.get("external_order_id")
        if external_order_id in (None, ""):
            external_order_id = get_primary_order_id(
                platform,
                str(raw.get("order_id") or raw.get("tid") or raw.get("orderId") or ""),
            )
        return {
            "after_sale_id": raw.get("after_sale_id") or raw.get("refund_id") or raw.get("refundId"),
            "canonical_after_sale_id": raw.get("canonical_after_sale_id") or raw.get("after_sale_id") or raw.get("refund_id") or raw.get("refundId"),
            ...
            "canonical_order_id": build_canonical_order_id(
                platform,
                str(external_order_id or raw.get("order_id") or raw.get("tid") or raw.get("orderId") or ""),
            ),
            "platform": platform,
            "status": raw.get("status", "unknown"),
            "status_text": raw.get("status_text") or self._get_status_text(raw.get("status", "unknown")),
