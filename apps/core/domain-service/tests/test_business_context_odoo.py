...
class FakeReplyGenerator:
    def generate_replies(self, context, intent=None):
        return [{"reply_type": "test", "content": "test", "score": 0.9}]

    def generate_actions(self, context):
        return [{"action_type": "test", "label": "Test"}]
...
class TestBusinessContextWithOdoo:
    def test_build_order_context_with_inventory(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="ORDER_001",
            biz_type="order",
    ...
    def test_build_order_context_without_inventory(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="ORDER_001",
            biz_type="order",
        ...
        assert context["biz_type"] == "order"
        assert (
            "inventory_snapshot" not in context
            or context.get("inventory_snapshot") is None
        )

    def test_build_order_context_with_order_audit(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="ORDER_001",
            biz_type="order",
    ...
    def test_build_order_context_with_exceptions(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="ORDER_002",
            biz_type="order",
        ...
        assert (
            context["order_exception_snapshots"][0]["exception_type"] == "address_issue"
        )

    def test_build_order_context_with_fulfillment(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="ORDER_001",
            biz_type="order",
    ...
    def test_build_conversation_context(self, context_resolver):
        context = context_resolver.build_context(
            platform="wecom_kf",
            biz_id="CONV_001",
    ...
    def test_build_after_sale_context(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
    ...
    def test_empty_inventory_returns_none(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
        ...
        assert (
            context.get("inventory_snapshot") is None
            or "inventory_snapshot" not in context
        )

    def test_empty_order_audit_returns_none(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="NONEXISTENT_ORDER",
            biz_type="order",
    ...
    def test_empty_exceptions_returns_empty_list(self, context_resolver):
        context = context_resolver.build_context(
            platform="taobao",
            biz_id="NONEXISTENT_ORDER",
            biz_type="order",
