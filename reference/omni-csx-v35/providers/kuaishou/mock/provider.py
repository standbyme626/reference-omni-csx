"""Kuaishou mock provider - calls official-sim-server for order/shipment/after-sale data."""
...
from providers.kuaishou.mock.mapper import map_order, map_shipment, map_after_sale
...
class KuaishouMockProvider(OrderProvider, ShipmentProvider, AfterSaleProvider):
    ...
    def get_order(self, order_id: str) -> OrderDTO:
        response = httpx.get(f"{self.base_url}/mock/kuaishou/orders/{order_id}", timeout=10)
        response.raise_for_status()
        return map_order(response.json(), "kuaishou")

    def get_shipment(self, order_id: str) -> ShipmentDTO:
        response = httpx.get(f"{self.base_url}/mock/kuaishou/shipments/{order_id}", timeout=10)
        response.raise_for_status()
        return map_shipment(response.json(), "kuaishou")

    def get_after_sale(self, after_sale_id: str) -> AfterSaleDTO:
        response = httpx.get(f"{self.base_url}/mock/kuaishou/after-sales/{after_sale_id}", timeout=10)
        response.raise_for_status()
        return map_after_sale(response.json(), "kuaishou")
