"""Taobao mock provider - calls official-sim-server for order/shipment/after-sale data."""
...
class TaobaoMockProvider(OrderProvider, ShipmentProvider, AfterSaleProvider):
    ...
    def get_order(self, order_id: str) -> OrderDTO:
        response = httpx.get(f"{self.base_url}/mock/taobao/orders/{order_id}", timeout=10)
        response.raise_for_status()
        return map_order(response.json(), "taobao")

    def get_shipment(self, order_id: str) -> ShipmentDTO:
        response = httpx.get(f"{self.base_url}/mock/taobao/shipments/{order_id}", timeout=10)
        response.raise_for_status()
        return map_shipment(response.json(), "taobao")

    def get_after_sale(self, after_sale_id: str) -> AfterSaleDTO:
        response = httpx.get(f"{self.base_url}/mock/taobao/after-sales/{after_sale_id}", timeout=10)
        response.raise_for_status()
        return map_after_sale(response.json(), "taobao")
