...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(`${API_GATEWAY_URL}/api/risk/cases${query ? `?${query}` : ""}`, {
    ...
    return NextResponse.json({ detail: "Failed to connect to risk cases service" }, { status: 500 });
  }
}
